 ...
 */
public class BasicUserModePortrait {
	 ...
	 */
	public BasicUserModePortrait(Context context, int flipInterval) {
		this.context = context;
		this.appSettings = ((OBDMeApplication)context.getApplicationContext()).getApplicationSettings();
		this.dataCollectorThread = ((OBDMeApplication)context.getApplicationContext()).getDataCollector();
		this.customFont = Typeface.createFromAsset(context.getAssets(), "LCD.ttf");  
			...
			for (Iterator<String> pidIrt = pollablePIDList.get(currentMode).iterator(); pidIrt.hasNext(); ) {
				...
				if (pidIrt.hasNext()) {
					...
					if (pidIrt.hasNext()) {
						...
						rootFlipper.addView(buildEnabledView(upperPID, lowerPID, currentView));
					}
					...
					//Otherwise there is only one PID left in the iterator
						...
						rootFlipper.addView(buildEnabledView(upperPID, currentView));
					}
				}
	 ...
	 * @return the view
	 */
	private View buildEnabledView(OBDPID upperPID, OBDPID lowerPID, Integer currentView) {
		...
		rootLinearLayout.addView(buildTitleView(upperPID));
		rootLinearLayout.addView(buildValueView(upperPID, currentView));

		//Add the lower title and value TextView objects to this linear layout
		rootLinearLayout.addView(buildTitleView(lowerPID));
		rootLinearLayout.addView(buildValueView(lowerPID, currentView));
	 ...
	 * @return the view
	 */
	private View buildEnabledView(OBDPID singlePID, Integer currentView) {
		...
		//Add the upper title and value TextView objects to this linear layout
		rootLinearLayout.addView(buildTitleView(singlePID));
		rootLinearLayout.addView(buildValueView(singlePID, currentView));
